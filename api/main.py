...
class CustomApp(FastAPI):
    def __init__(self):
        super().__init__(
            ...
            openapi_tags=[
                ...
                {"name": "TikTok", "description": "TikTok related endpoints"},
                {"name": "Roblox", "description": "Roblox related endpoints"},
                {"name": "Snapchat", "description": "Snapchat related endpoints"},
                {"name": "Instagram", "description": "Instagram related endpoints"},
                {"name": "Spotify", "description": "Spotify related endpoints"},
                {"name": "LastFM", "description": "Last FM service related endpoints"},
                {"name": "Discord", "description": "Discord related endpoints"},
                {"name": "OpenAI", "description": "Interact with OpenAI services"},
                {"name": "Media", "description": "Image related endpoints"},
                {"name": "Roleplay", "description": "Express yourself using gifs"},
                {
                    "name": "Master",
    ...
    async def startup(self):
        ...
        for api_key in api_keys:
            api = APIKey(key=api_key.key, user_id=api_key.user_id, role=api_key.role)
    ...
    async def read_url(self, url: str, content_type: Optional[List[str]] = None):
        ...
        if not re.search(regex, url):
            raise HTTPException(
                status_code=400, detail="The parameter provided is not an url"
            )
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(url) as r:
                ...
                if int(r.headers.get("Content-Length")) > 5000000:
                    raise HTTPException(
                        status_code=400, detail="Image size is bigger than 5 MB"
                    )
    ...
    async def screenshot(self, url: str, timeout: int):
        ...
        browser = await pyppeteer.launch(
            headless=True, defaultViewport=viewport, args=["--no-sandbox"]
        )
        ...
        if content_type := r.headers.get("content-type"):
            if not any([m in content_type for m in ["text/html", "application/json"]]):
                raise HTTPException(
                    status_code=400, detail="Not allowed to screenshot this page"
                )
            ...
            if any(
                ...
                raise HTTPException(
                    status_code=403, detail="This website contains explicit content"
                )
...
class AuthScheme(APIKeyHeader):
    ...
    async def __call__(self, request: Request) -> str:
        ...
        logging.info(
            f"{api.user_id} ({api.role}) made a request using the key: {api.key} on {request.url}"
        )
...
def generate_api_sig(params: dict, secret: str) -> str:
    ...
    logging.info(
        f"Signature base string: {signature_base_string}"
    )  # Log the base string used for signature
...
async def authorize(discord_user_id: str):
    redirect_uri = (
        f"https://api.akari.bot/lastfm/callback?discord_user_id={discord_user_id}"
    )
...
async def lastfm_callback(request: Request, token: str, discord_user_id: str):
    ...
    logging.info(
        f"Request params with api_sig: {params}"
    )  # Log the request parameters with api_sig
    logging.info(
        f"Generated API signature: {api_sig}"
    )  # Log the generated API signature
        ...
        async with session.get(
            ...
            try:
                ...
                if error_element is not None:
                    ...
                    logging.error(
                        f"Invalid response from Last.fm API: {error_message_text}"
                    )  # Log the error response
                    raise HTTPException(
                        status_code=400,
                        detail=f"Invalid response from Last.fm API: {error_message_text}",
                    )
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Failed to parse XML response from Last.fm API",
                )
            except AttributeError as e:
                logging.error(f"Missing expected elements in XML response: {e}")
                raise HTTPException(
                    status_code=500,
                    detail="Missing expected elements in XML response from Last.fm API",
                )
    ...
    if access_token := await oauth.exchange_code(code):
        if user := await oauth.get_user(access_token):
            if guilds := await oauth.get_user_guilds(access_token):
                if current_guild := next((g for g in guilds if g["id"] == state), None):
                    await oauth.add_roles_for(user["id"], current_guild["id"])
                ...
                return templates.TemplateResponse(
                    "error.html",
                    {"request": request, "message": "You are not in this guild"},
                )

    return templates.TemplateResponse(
        "error.html", {"request": request, "message": "Unable to verify you"}
    )
...
async def instagram_story(username: str, token=Depends(auth_scheme)):
    ...
    if payload == 404:
        raise HTTPException(status_code=404, detail="Instagram story not found")
    return payload
...
async def spotify_song(
    request: Request, url: str, token=Depends(auth_scheme)
) -> JSONResponse:
    ...
    async with app.locks[token.key]:
        ...
        async with cs.get(
            "https://api.fabdl.com/spotify/get", params={"url": url}
        ) as r:
            x = await r.json()
        ...
        if result := x.get("result"):
            ...
            async with aiohttp.ClientSession(headers=headers) as cs:
                async with cs.get(
                    f"https://api.fabdl.com/spotify/mp3-convert-task/{gid}/{id}"
                ) as r:
                    y: dict = await r.json()
            ...
            if re := y.get("result"):
                ...
                async def recursive_download_url(tid: str) -> str:
                    async with aiohttp.ClientSession(headers=headers) as session:
                        async with session.get(
                            f"https://api.fabdl.com/spotify/mp3-convert-progress/{tid}"
                        ) as res:
                            d = await res.json()
        ...
        raise HTTPException(status_code=400, detail="Unable to download song")


@app.get("/lastfm/albumplays", tags=["Audio"], response_model=LastFmAlbumPlays)
async def lastfm_album_plays(
    username: str, album: str, artist: str, token=Depends(auth_scheme)
):
    """Get the amount of plays a Last FM user has on a specific album"""

    results = await app.lastfm.get_album_info(username, album, artist)
...
async def lastfm_recent_tracks(
    username: str, tracks: int = 1, token=Depends(auth_scheme)
):
...
async def lastfm_chart(
    ...
    key = next(
        (
            k
            for k in app.cache.payload.keys()
            if k.startswith(f"lastfm_{size}_{period}_{username}")
        ),
        None,
    )

    if key:
        return {"image_url": str(request.url_for("cdn", key=key, format="png"))}
    ...
    async with app.locks[f"lastfm-{username}"]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Wrong size format. Size example (3x3)"
            )
        ...
        if album_count > 50:
            raise HTTPException(
                status_code=400, detail="Your chart cannot contain more than 50 albums"
            )

        results = await app.lastfm.get_recents(
            username, period, album_count, "user.gettopalbums"
        )
        ...
        buffer.seek(0)
        key = f"lastfm_{size}_{period}_{username}_" + "".join(
            secrets.choice(app.characters) for _ in range(8)
        )
        await app.cache.set(key, buffer.read(), 3600)
...
async def chatgpt(
    prompt: str, version: float = 3.5, token: APIKey = Depends(auth_scheme)
):
        ...
        async with aiohttp.ClientSession(headers=headers) as cs:
            async with cs.post(
                ...
                try:
                    ...
                    await app.cache.set(f"chatgpt-{prompt.lower().strip()}", response)
                    return {"response": response}
                except:
                    raise HTTPException(
                        status_code=429, detail="Too many requests to chatgpt"
                    )
...
async def discord_user_avatar_post(body: DiscordAvatarPost, token=Depends(auth_scheme)):
    ...
    if token.role not in ["master", "bot_developer"]:
        raise HTTPException(
            status_code=403, detail="You do not have access to this endpoint"
        )
    ...
    async with app.locks[1]:
        ...
        await app.cache.set(f"avatars-{body.url}", True)
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.post(
                "https://images.akari.bot/upload", json=body.dict()
            ) as r:
                return await r.text()


@app.get("/avatars/{user_id}", tags=["Socials"])
async def discord_user_avatars(
    request: Request, user_id: int, token=Depends(auth_scheme)
):
    """
    ...
    async with aiohttp.ClientSession() as cs:
        async with cs.get(f"https://api.akari.bot/avatarhistory/{user_id}") as r:
            ...
            await app.cache.set(f"user-avatars-{user_id}", user_avatars, 600)

            return user_avatars


@app.get(
    "/discord/user/profile", tags=["Socials"], response_model=DiscordUserProfileModel
)
async def discord_user_profile(
    user_id: int, guild_id: Optional[int] = None, api_key: str = Header(None)
):
    """Get a discord user's information"""

    token = os.getenv("DISCORD_BOT_TOKEN")
    logging.info(
        f"Fetching profile for user_id: {user_id}, guild_id: {guild_id}, token: {token}"
    )
    ...
    if not app.worker.check_token(token):
        ...
        raise HTTPException(
            status_code=401, detail="Given token is invalid or not a discord user token"
        )
    ...
    if payload == token:
        logging.warning(
            f"Couldn't fetch details about user_id: {user_id}, guild_id: {guild_id}"
        )
        raise HTTPException(
            status_code=404, detail="Couldn't fetch details about this user"
        )
    else:
        payload = None
    ...
    if not payload:
        try:
            user_data = await app.socials.discord.get_user(
                token, str(user_id), str(guild_id)
            )
        ...
        await app.cache.set(f"discorduser-{user_id}-{guild_id}", payload.dict(), 3600)
...
async def roleplay_bite(request: Request, token=Depends(auth_scheme)):
    ...
    await app.cache.set(key, path[:-4])

    return {"image_url": str(request.url_for("cdn", key=key, format="gif"))}
...
async def cdn(key: str, format: str):
    ...
    if cache := app.cache.get(key):
        if isinstance(cache, str):
            return FileResponse(f"{cache}.{format}")
        else:
            return Response(content=cache, media_type=f"image/{format}")
    else:
...
async def video_to_gif(request: Request, url: str, token=Depends(auth_scheme)):
    ...
    async with app.locks[token.user_id]:
        regex = r"(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))"
        if not re.search(regex, url):
            raise HTTPException(status_code=400, detail="This is not an url")

        async with aiohttp.ClientSession() as cs:
            async with cs.get(url) as r:
                if r.headers.get("Content-Type") not in [
                    "video/mp4",
                    "video/mov",
                    "video/quicktime",
                ]:
                    raise HTTPException(
                        status_code=400, detail="URL must be an mp4 or mov file"
                    )
                ...
                await app.cache.set(key, gif_path[:-4])
...
async def pictures(
    ...
    path = os.path.join(
        "/root/AkariBot/api/images", type.capitalize(), category.capitalize()
    )
    ...
    if format:
        dir_pictures = (
            [d for d in dir_pictures if d.endswith("gif")]
            if format == "gif"
            else [d for d in dir_pictures if not d.endswith("gif")]
        )
    ...
    key = next(
        (
            k
            for k, v in app.cache.payload.items()
            if v == f"{type.capitalize()}/{category.capitalize()}/{img[:-4]}"
        ),
        None,
    )
    ...
    if not key:
        ...
        await app.cache.set(
            key,
            f"/root/AkariBot/api/images/{type.capitalize()}/{category.capitalize()}/{img[:-4]}",
        )
...
async def website_screenshot(
    request: Request, url: str, timeout: int = 1, token=Depends(auth_scheme)
) -> JSONResponse:
    """
    ...
    if re.search(regex, url):
        key = next(
            (
                ...
                for k, v in app.cache.payload.items()
                if v
                == f"./screenshots/{url.replace('https://', '').replace('http://', '').replace('/', '')}"
            ),
            None,
        )
        ...
        if key:
            return {
                "website_url": url,
                "screenshot_url": request.url_for(
                    "cdn", key=key, format="png"
                ).__str__(),
            }
        ...
        if website_path := await app.screenshot(url, timeout):
            ...
            await app.cache.set(key, website_path)
    ...
    raise HTTPException(status_code=400, detail=f"Unable to screenshot {url}")


@app.get("/transparent", response_model=TransparentModel, tags=["Media"])
async def transparent(
    request: Request, url: str, token: APIKey = Depends(auth_scheme)
) -> JSONResponse:
    ...
    async with app.locks[str(token)]:
        ...
        temp_file_output = os.path.join(
            "./transparent/", f"transparent_output_{random_char}.png"
        )
        ...
        try:
            ...
            print(f"exception: {e}")
            raise HTTPException(
                status_code=400, detail="Unable to make this image transparent"
            )
        ...
        key = "".join(random.choice(app.characters) for _ in range(12))
        await app.cache.set(key, temp_file_output[:-4])

        return {"image_url": request.url_for("cdn", key=key, format="png").__str__()}
...
async def api_create(form: APIForm, token=Depends(auth_scheme)) -> JSONResponse:
    ...
    if token.role != "master":
        raise HTTPException(
            status_code=403, detail="You are not allowed to use this endpoint"
        )

    key = await app.state.db.fetchval(
        "SELECT key FROM api_key WHERE user_id = $1", form.user_id
    )
    ...
    api = APIKey(key=key, user_id=form.user_id, role=form.role)
...
async def api_revoke(token=Depends(auth_scheme)) -> JSONResponse:
    ...
    await app.state.db.execute("DELETE FROM api_key WHERE key = $1", str(token))

    return Response(status_code=204)


if __name__ == "__main__":
    app.run("main:app", host="0.0.0.0", port=5500, reload=True)
