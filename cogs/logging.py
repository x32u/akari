...
class UserID(discord.ui.Button):
    def __init__(self, id_name: str):
        super().__init__(
            label=id_name, 
            custom_id="logs:user_id"
        )
    
    async def callback(self, interaction: discord.Interaction):
        footer = interaction.message.embeds[0].footer.text
        message = footer.split(" | ")[1][len("ID: "):] if footer.find("|") != -1 else footer[len("ID: "):]
...
class UserBan(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if member := interaction.guild.get_member(user_id): 
            await member.ban(reason=f"Banned by {interaction.user} from logging message")
...
class Logs(commands.Cog):
    ...
    async def on_bulk_message_delete(self, messages: List[discord.Message]):
        ...
        if record := await self.bot.db.fetchval("SELECT messages FROM logging WHERE guild_id = $1", guild.id):
    ...
    async def on_audit_log_entry_create(self, entry: discord.AuditLogEntry):
        if record := await self.bot.db.fetchrow("SELECT * FROM logging WHERE guild_id = $1", entry.guild.id):
            async with self.locks[entry.guild.id]:
                ...
                match entry.action.name: 
                    ...
                    case 'channel_update':
                        if channel := entry.guild.get_channel(record.channels):
                            if getattr(entry.before, 'name', None) != getattr(entry.after, 'name', None): 
                                embed.description = f"{entry.target.mention} got a different name"
                                embed.add_field(
                                    name="Before",
                                ...
                                .add_field(
                                    name="After",
                                    value=entry.after.name,
                                    inline=False
                                )
                            elif getattr(entry.before, 'topic', None) != getattr(entry.after, 'topic', None): 
                                embed.description = f"{entry.target.mention} got a different topic"
                                embed.add_field(
                                    name="Before",
                    ...
                    case 'bot_add':
                        if channel := entry.guild.get_channel(record.members):
                            embed.description = f"{entry.target.mention} was added to this server"
                            embed.set_author(
                                name=str(entry.target),
                                icon_url=entry.target.display_avatar.url
                            )\
                    ...
                    case 'role_create': 
                        if channel := entry.guild.get_channel(record.roles):
                            embed.color = entry.target.color if entry.target.color.value != 0 else self.bot.color
                            embed.add_field(
                                name="Role", 
                                value=f"{entry.target.mention} (`{entry.target.id}`)", 
                                inline=False
                    ...
                    case 'guild_update':
                        if channel := entry.guild.get_channel(record.guild):
                            ...
                            embed.set_author(
                                name=str(entry.guild),
                                icon_url=entry.guild.icon
                            )\
                            .add_field(
                                name="Moderator",
                                value=f"**{entry.user}** (`{entry.user.id}`)",
                    ...
                    case 'member_role_update':
                        if channel := entry.guild.get_channel(record.members):
                            embed.description = f"{entry.target.mention}'s roles got updated"
                            embed.add_field(
                                name="Moderator",
                                value=f"**{entry.user}** (`{entry.user.id}`)",
                                inline=False
                            ...
                            if entry.changes.after.roles: 
                                embed.add_field(
                                    name="Added Roles",
                                    value=", ".join([r.mention for r in entry.changes.after.roles]) 
                                    if len(entry.changes.after.roles) < 6 
                                    else ', '.join([r.mention for r in entry.changes.after.roles[:5]]) + f" (+ {len(entry.changes.after.roles)-5} more)",
                                    inline=False
                                )
                            ...
                            if entry.changes.before.roles:
                                embed.add_field(
                                    name="Removed Roles",
                                    value=", ".join([r.mention for r in entry.changes.before.roles]) 
    ...
    async def logs_settings(self, ctx: Context):
        ...
        if record := await self.bot.db.fetchrow("SELECT * FROM logging WHERE guild_id = $1", ctx.guild.id):
            ...
            if not statistics: 
                return await ctx.error("Nothing to display") 
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def logs_messages(self, ctx: Context, *, channel: Union[discord.TextChannel, str]):
        """
        ...
        if isinstance(channel, str):
            if channel.lower().strip() in ['none', 'remove']:
                if await self.bot.db.fetchval(
                    "SELECT messages FROM logging WHERE guild_id = $1",
                    ctx.guild.id
                ):
                    ...
                    return await ctx.success("No longer logging **messages**")
                else:
                    return await ctx.error("Message logging is **not** enabled")
            else: 
        ...
        return await ctx.success(f"Sending **message logs** to {channel.mention}")
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def logs_guild(self, ctx: Context, *, channel: Union[discord.TextChannel, str]):
        """
        ...
        if isinstance(channel, str):
            if channel.lower().strip() in ['none', 'remove']:
                if await self.bot.db.fetchval(
                    "SELECT guild FROM logging WHERE guild_id = $1",
                    ctx.guild.id
                ):
                    await self.bot.db.execute(
                    ...
                    return await ctx.success("No longer logging **guild events**")
                else:
                    return await ctx.error("Guild logging is **not** enabled")
            else:
                raise commands.ChannelNotFound(channel)
        ...
        return await ctx.success(f"Sending **guild logs** to {channel.mention}")
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def logs_roles(self, ctx: Context, *, channel: Union[discord.TextChannel, str]):
        """
        ...
        if isinstance(channel, str):
            if channel.lower().strip() in ['none', 'remove']:
                if await self.bot.db.fetchval(
                    "SELECT roles FROM logging WHERE guild_id = $1",
                    ctx.guild.id
                ):
                    await self.bot.db.execute(
                    ...
                    return await ctx.success("No longer logging **roles**")
                else:
                    return await ctx.error("Roles logging is **not** enabled")
            else: 
                raise commands.ChannelNotFound(channel)
        ...
        return await ctx.success(f"Sending **role logs** to {channel.mention}")
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def logs_channels(self, ctx: Context, *, channel: Union[discord.TextChannel, str]):
        """
        ...
        if isinstance(channel, str): 
            if channel.lower().strip() in ['none', 'remove']:
                if await self.bot.db.fetchval(
                    "SELECT channels FROM logging WHERE guild_id = $1",
                    ctx.guild.id
                ):
                    await self.bot.db.execute(
                    ...
                    return await ctx.success("No longer logging **channels**")
                else:
                    return await ctx.error("Channels logging is **not** enabled")
            else: 
                raise commands.ChannelNotFound(channel)
        ...
        return await ctx.success(f"Sending **channel logs** to {channel.mention}")
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def logs_members(self, ctx: Context, *, channel: Union[discord.TextChannel, str]):
        """
        ...
        if isinstance(channel, str):
            if channel.lower().strip() in ['none', 'remove']:
                if await self.bot.db.fetchval(
                    "SELECT members FROM logging WHERE guild_id = $1",
                    ctx.guild.id
                ):
                    await self.bot.db.execute(
                    ...
                    return await ctx.success("No longer logging **members**")
                else:
                    return await ctx.error("Members logging is **not** enabled")
            else:
                raise commands.ChannelNotFound(channel)
        ...
        return await ctx.success(f"Sending **member logs** to {channel.mention}")
