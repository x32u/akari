...
class Reactions(Cog):
   ...
   if check: 
    ...
    if role: 
     if role.is_assignable():
      if not role in self.bot.get_guild(payload.guild_id).get_member(payload.user_id).roles: 
       await self.bot.get_guild(payload.guild_id).get_member(payload.user_id).add_roles(role, reason="Reaction Role")
  ...
  async def on_reactionrole_remove(self, payload: discord.RawReactionActionEvent):
   ...
   if check: 
    ...
    if role: 
     if role.is_assignable():
      if role in self.bot.get_guild(payload.guild_id).get_member(payload.user_id).roles: 
       await self.bot.get_guild(payload.guild_id).get_member(payload.user_id).remove_roles(role, reason="Reaction Role")    
    ...
    if str(payload.emoji) == res['emoji']:
     ...
     if len(reactions) > 0:
      ...
      if not check: 
       ...
       file = None
       if not mes.embeds:
        embed = discord.Embed(color=self.bot.color, description=mes.content, timestamp=mes.created_at)
        ...
        if mes.attachments: 
         if mes.attachments[0].filename.endswith(("png", "jpeg", "jpg")):
          embed.set_image(url=mes.attachments[0].proxy_url)
           ...
           file = discord.File(fp=io.BytesIO(await mes.attachments[0].read()), filename=mes.attachments[0].filename) 
       else: 
        em = mes.embeds[0]
        embed = discord.Embed(color=em.color, description=em.description or mes.content, title=em.title, url=em.url)
        
        if em.author: 
         embed.set_author(name=em.author.name, icon_url=em.author.proxy_icon_url, url=em.author.url)
        else: 
         embed.set_author(name=mes.author, icon_url=mes.author.display_avatar.url)  
        ...
        if em.footer: 
         embed.set_footer(text=em.footer.text, icon_url=em.footer.icon_url)
       ...
       view.add_item(discord.ui.Button(label="message", url=mes.jump_url))
