...
class EmbedBuilder(): 
  ...
  def is_url(self, text: str, parameter: str):
    ...
    if not re.search(regex, text):
      raise commands.BadArgument(f"The **{parameter}** parameter got an invalid url")
    ...
    if embeds := message.embeds:
     ...
     if embed.get('fields'):
      for field in embed['fields']:
        to_return += "$v{field: " + f"name: {field['name']} && value: {field['value']}{' && inline' if field['inline'] else ''}" + "}"
...
class EmbedScript(commands.Converter): 
  ...
  async def convert(self, ctx: commands.Context, argument: str):
   ...
   if x[0] or x[1]: 
    if x[3]:
      return {"content": x[0], "embed": x[1], "view": x[2], "delete_after": x[3]} 
    else: 
