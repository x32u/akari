...
class EmbedBuilding(View):
  ...
  def replace_images(self, interaction: Interaction, params: str) -> str: 
    ...
    if interaction.user.display_avatar.url in params: 
      params = params.replace(interaction.user.display_avatar.url, "{user.avatar}")
  ...
  async def save_embed(self, interaction: Interaction, button: Button):
    ...
    if embed.author: 
      ...
      if embed.author.name and embed.author.icon_url: 
       mes += "name: " + embed.author.name + " && " + "icon: " + self.replace_images(interaction, embed.author.icon_url) + "}" 
       ...
       mes += "icon: " + self.replace_images(interaction, embed.author.icon_url) + "}"
    if embed.image: 
      mes += "$v{image: " + self.replace_images(interaction, embed.image.url) + "}"
    if embed.thumbnail: 
      mes += "$v{thumbnail: " + self.replace_images(interaction, embed.thumbnail.url) + "}"  
    if embed.footer: 
      mes += "$v{footer: "
      if embed.footer.text and embed.footer.icon_url: 
        mes += "text: " + embed.footer.text + " && " + "icon: " + self.replace_images(interaction, embed.footer.icon_url) + "}"
    ...
    await interaction.response.edit_message(content=f"```{mes}```", embed=None, view=None)