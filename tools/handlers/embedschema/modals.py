...
class FooterModal(Modal, title="Edit your embed"):
  ...
  async def on_error(self, interaction: Interaction, error: Exception) -> None:
   await interaction.warn(f"A problem occured while trying to edit your embed - {error}", ephemeral=True)   
...
class AuthorModal(Modal, title="Edit your embed"):
  ...
  async def on_submit(self, interaction: Interaction) -> None: 
   ...
   embed.set_author(name=name, icon_url=self.variable_replace(interaction.user, icon))
   await interaction.response.edit_message(embed=embed)
...
class BasicModal(Modal, title="Edit your embed"):
  ...
  desc = TextInput(label="description", style=TextStyle.long, max_length=2000, required=False)
  color = TextInput(label="hex color", required=False, max_length=7)
  timestamp = TextInput(label="timestamp", placeholder="either yes or no. nothing else", required=False)
