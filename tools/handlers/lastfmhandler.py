...
class Spotify(Requests): 
  ...
  async def get_token_from_code(self, code: str):
    ...
    result = await self.post_request("https://accounts.spotify.com/api/token", headers=headers, params=data)
    if result[0] == 200:
     return result[1]['access_token'] 
  
  async def get_access_token(self): 
   access_token = await self.bot.db.fetchrow("SELECT access_token FROM spotify WHERE user_id = $1", 371224177186963460)
   return await self.get_token_from_code(access_token[0])
  
  async def search(self, query: str): 
   access_token = await self.get_access_token()
   result = await self.get_request("https://api.spotify.com/v1/search", headers={"Authorization": f"Bearer {access_token}"}, params={"q": query, "type": ["track"], "limit": 1})
