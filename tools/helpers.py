...
def guild_perms(**perms: bool) -> Any:
 ...
 async def predicate(ctx: PretendContext):
  ...
  if not any(p in author_permissions for p in perms): 
   ...
   results = await ctx.bot.db.fetch(f"SELECT perms FROM fake_perms WHERE guild_id = $1 AND role_id IN ({roles})", ctx.guild.id)
...
class CustomInteraction(Interaction):
  ...
  async def error(self, message: str, ephemeral: bool=False) -> None: 
   return await self.response.send_message(embed=Embed(color=self.client.no_color, description=f"{self.client.no} {self.user.mention}: {message}"), ephemeral=ephemeral)

  async def warn(self, message: str, ephemeral: bool=False) -> None: 
   return await self.response.send_message(embed=Embed(color=self.client.warning_color, description=f"{self.client.warning} {self.user.mention}: {message}"), ephemeral=ephemeral)
...
class PretendHelp(Help):
 ...
 async def send_command_help(self, command: Command):
  ...
  embed.add_field(name="usage", value=f"```{self.context.clean_prefix}{command.qualified_name} {' '.join([f'[{a}]' for a in command.clean_params]) if command.clean_params != {} else ''}\n{command.usage or ''}```", inline=False)
  if command.aliases: 
...
class PretendContext(Context): 
  ...
  async def get_sticker(self) -> StickerItem: 
   ...
   messages = [message async for message in self.channel.history(limit=20) if message.stickers] 
   if len(messages) > 0: 
  ...
  async def paginator(self, embeds: List[Union[Embed, str]]) -> Message: 
   ...
   paginator.add_button('delete', emoji="<:stop:1018156487232720907>",  style=ButtonStyle.red)
