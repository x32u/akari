...
async def bump_remind(bot): 
  results = await bot.db.fetch("SELECT channel_id, reminder, user_id, time FROM bumpreminder WHERE time IS NOT NULL")
  for result in [r for r in results if r[3].timestamp() < datetime.datetime.now().timestamp()]: 
    ...
    if channel: 
      ...
      try:
        ...
        continue
    await bot.db.execute("UPDATE bumpreminder SET time = $1, channel_id = $2, user_id = $3 WHERE channel_id = $4", None, None, None, result[0]) 
  ...
  for result in results: 
    if result['till']:
      if datetime.datetime.now().timestamp() > result['till'].timestamp(): 
        ...
        val = await bot.db.fetchrow("SELECT * FROM authorize WHERE user_id = $1", user)
        if not val: 
        ...
        if guild: 
          await guild.leave()
          await bot.get_channel(1122993923422429274).send(f"Left **{guild.name}** (`{guild.id}`). monthly payment not received")
        else: 
          await bot.get_channel(1122993923422429274).send(f"Removing `{result['guild_id']}`. monthly payment not received") 
  ...
  if channel := bot.get_channel(channel_id):
    ...
    try:
      ...
      if len(members) <= winners:       
        embed = Embed(color=bot.color, title=message.embeds[0].title, description=f"Hosted by: <@!{result['host']}>\n\nNot enough entries to determine the winners!")
