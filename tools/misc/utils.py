...
class Timezone():
  ...
  async def set_timezone(self, member: Member, location: str) -> str:
    ...
    timezone = await self.bot.loop.run_in_executor(self.bot.executor, lambda: obj.timezone_at(**kwargs))
    local = arrow.utcnow().to(timezone).naive
    ...
    payload = {
      'timezone': timezone,
      'date': f"{week_day} {month} {day} {hour}"
    }
...
class BdayMember(MemberConverter):
  async def convert(self, ctx: PretendContext, argument: str):
    ...
    check = await ctx.bot.db.fetchrow("SELECT * FROM bday WHERE user_id = $1", member.id)
    if not check: 
      raise BadArgument(f"{f'**You** don' if member == ctx.author else f'**{member.name}** doesn'}'t have a **birthday** configured")
...
class Birthday():
  ...
  async def set_bday(self, member: Member, month: str, day: str) -> Tuple[str, str]: 
   check = await self.bot.db.fetchrow("SELECT * FROM bday WHERE user_id = $1", member.id)
   ...
   if not check:
    ...
    await self.bot.db.execute("UPDATE bday SET month = $2, day = $3 WHERE user_id = $1", *args)
