...
from discord import Interaction, Embed, ButtonStyle, Button, Member, TextStyle, PartialEmoji, Sticker, HTTPException, File, Emoji
  ...
  async def on_submit(self, interaction: Interaction) -> None:
    ...
    if check:
      if re.search(r"[(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)", self.name.value):
        return await interaction.response.send_message("You cannot use links in a confession", ephemeral=True)
...
class Donate(View):
  ...
  async def button_callback(self, interaction: Interaction, button: Button):
   await interaction.response.send_message(self.responses.get(button.custom_id), ephemeral=True) 
...
class BoosterMod(View): 
 ...
 async def yes_button(self, interaction: Interaction, button: Button):  
   if self.ctx.command.name == 'ban':
    ...
    return await interaction.response.edit_message(embed=Embed(color=interaction.bot.yes_color, description=f"{interaction.client.yes} {interaction.user.mention}: Banned {self.member.mention} - {self.reason}"), view=None)
   else: 
    await self.member.kick(reason=self.reason)
    return await interaction.response.edit_message(embed=Embed(color=interaction.bot.yes_color, description=f"{interaction.client.yes} {interaction.user.mention}: Kicked {self.member.mention} - {self.reason}"), view=None)

 @button(label="Decline", style=ButtonStyle.red)
 async def no_button(self, interaction: Interaction, button: Button): 
  if self.ctx.author.id != interaction.user.id: return await interaction.response.send_message(embed=Embed(color=interaction.client.warning_color, description=f"{interaction.client.warning_emoji} {interaction.user.mention}: You are **not** the author of this embed"), ephemeral=True)
  await interaction.response.edit_message(embed=Embed(color=interaction.client.color, description="Cancelling action..."), view=None)
...
class MarryView(View): 
   ...
   async def interaction_check(self, interaction: Interaction):
    if interaction.user == self.ctx.author: 
     await interaction.warn("You cannot interact with your own marriage", ephemeral=True)
     return False 
   ...
   async def yes(self, interaction: Interaction, button: Button): 
    if await interaction.client.db.fetchrow("SELECT * FROM marry WHERE $1 IN (author, soulmate)", self.ctx.author.id): 
      return await interaction.client.send_message(f"{self.ctx.author.mention} already accepted a marriage", ephemeral=True)
      
    if await interaction.client.db.fetchrow("SELECT * FROM marry WHERE $1 IN (author, soulmate)", interaction.user.id): 
      return await interaction.client.send_message("You **already accepted a marriage", ephemeral=True)
    ...
    embe = Embed(color=self.marry_color, description=f"{self.wedding} {self.ctx.author.mention} succesfully married with {self.member.mention}")
   ...
   async def no(self, interaction: Interaction, button: Button):                         
    embe = Embed(color=self.marry_color, description=f"{self.ctx.author.mention} i'm sorry, but {self.member.mention} is probably not the right person for you")
    await interaction.response.edit_message(content=None, embed=embe, view=None)
    self.status = True 
   
   async def on_timeout(self):
     if self.status == False:
      embed = Embed(color=0xff819f, description=f"{self.member.mention} didn't reply in time :(")  
      try: 
...
class Transfer(View): 
 ...
 @button(label="Approve", style=ButtonStyle.success)
 async def yes_button(self, interaction: Interaction, button: Button): 
  user_cash = (await interaction.client.db.fetchrow("SELECT cash FROM economy WHERE user_id = $1", self.to.id))[0] 
  author_cash = (await interaction.client.db.fetchrow('SELECT cash FROM economy WHERE user_id = $1', interaction.user.id))[0]
  if author_cash < self.amount: 
   embed = Embed(color=interaction.client.no_color, description=f"{interaction.client.no} {interaction.user.mention}: You do no have enough money to transfer")
   await interaction.response.edit_message(embed=embed, view=None)
   self.confirmed = True
   return
  
  await interaction.client.db.execute("UPDATE economy SET cash = $1 WHERE user_id = $2", round(user_cash+self.amount, 2), self.to.id)
  await interaction.client.db.execute("UPDATE economy SET cash = $1 WHERE user_id = $2", round(author_cash-self.amount, 2), interaction.user.id) 
  embed = Embed(color=0xd3d3d3, description=f"{interaction.user.mention}: Transfered **{self.amount}** to {self.to.mention}")
  await interaction.response.edit_message(embed=embed, view=None)
  self.confirmed = True 
 ...
 async def on_timeout(self): 
  if not self.confirmed: 
   embed = Embed(color=self.ctx.bot.color, description="Transfer is now canceled")
   await self.message.edit(embed=embed, view=None) 

class DownloadAsset(View):
    def __init__(self: "DownloadAsset", ctx: Context, asset: Union[PartialEmoji, Emoji, Sticker]):
      super().__init__()
      self.ctx = ctx 
      self.asset = asset
      self.pressed = False
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
      ...
      if not interaction.user.guild_permissions.manage_expressions:
        await interaction.warn("You do not have permissions to add emojis/stickers in this server", ephemeral=True)
        return False 
      
      if not interaction.user.guild.me.guild_permissions.manage_expressions:
       await interaction.warn("The bot doesn't have permissions to add emojis/stickers in this server", ephemeral=True)
       return False
