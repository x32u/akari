...
from discord import Embed, Interaction, TextStyle, SelectOption, ButtonStyle, Guild, utils, VoiceChannel  
...
class InteractionMes: 
  async def vc_task(interaction: Interaction) -> bool: 
   ...
   re = await interaction.client.db.fetchrow("SELECT channel_id FROM voicemaster WHERE guild_id = $1", interaction.guild.id)
   if not re: 
     await interaction.warn("VoiceMaster is **not** configured", ephemeral=True)
     return False
   channel = interaction.guild.get_channel(re[0])
   if not channel: 
     await interaction.warn("VoiceMaster main channel **not** found", ephemeral=True)
     return False 
   if interaction.user.voice.channel.category_id != channel.category_id: 
     await interaction.warn("You are not in a voice channel created by the bot", ephemeral=True)
     return False 
   check = await interaction.client.db.fetchrow("SELECT user_id FROM vcs WHERE voice = $1", interaction.user.voice.channel.id)
   if check[0] != interaction.user.id: 
     await interaction.warn("You are **not** the **owner** of this voice channel", ephemeral=True)
     return False 
   return True 
...
class lock(Button): 
  ...
  async def callback(self, interaction: Interaction) -> None: 
   if await InteractionMes.vc_task(interaction): 
    ...
    return await interaction.approve(f"Locked {interaction.user.voice.channel.mention}", ephemeral=True)
...
class unlock(Button): 
  ...
  async def callback(self, interaction: Interaction) -> None: 
   if await InteractionMes.vc_task(interaction): 
    ...
    return await interaction.approve(f"Unlocked {interaction.user.voice.channel.mention}", ephemeral=True) 
...
class hide(Button): 
 ...
 async def callback(self, interaction: Interaction) -> None: 
  if await InteractionMes.vc_task(interaction): 
    ...
    return await interaction.approve(f"Hidden {interaction.user.voice.channel.mention}", ephemeral=True) 
...
class reveal(Button): 
 ...
 async def callback(self, interaction: Interaction) -> None: 
  if await InteractionMes.vc_task(interaction): 
    ...
    return await interaction.approve(f"Revealed {interaction.user.voice.channel.mention}", ephemeral=True)
 ...
 async def callback(self, interaction: Interaction) -> None: 
  if await InteractionMes.vc_task(interaction): 
    if interaction.user.voice.channel.user_limit == 0: 
     return await interaction.warn("Limit cannot be lower than 0", ephemeral=True)
    ...
    return await interaction.approve(f"Decreased {interaction.user.voice.channel.mention}'s member limit", ephemeral=True)
 ...
 async def callback(self, interaction: Interaction) -> None: 
  if await InteractionMes.vc_task(interaction): 
    if interaction.user.voice.channel.user_limit == 99: 
     return await interaction.warn("Limit cannot be higher than 99", ephemeral=True)
    ...
    return await interaction.approve(f"Increased {interaction.user.voice.channel.mention}'s member limit", ephemeral=True)
...
class claim(Button): 
 ...
 async def callback(self, interaction: Interaction) -> None: 
  if not interaction.user.voice:
   return await interaction.warn("You are **not** in a voice channel", ephemeral=True)
  check = await interaction.client.db.fetchrow("SELECT user_id FROM vcs WHERE voice = $1", interaction.user.voice.channel.id)
  if interaction.guild.get_member(check[0]) in interaction.user.voice.channel.members: 
   return await interaction.warn("The owner is still in the voice channel", ephemeral=True)
  await interaction.client.db.execute("UPDATE vcs SET user_id = $1 WHERE voice = $2", interaction.user.id, interaction.user.voice.channel.id)
  return await interaction.approve("You are the new owner of the voice channel", ephemeral=True)

class kick(Button): 
  def __init__(self, label, emoji, style): 
   super().__init__(label=label, emoji=emoji, style=style, custom_id="persistent_view:kick")
  ...
  async def callback(self, interaction: Interaction) -> None: 
   if await InteractionMes.vc_task(interaction): 
    ...
    options = [SelectOption(label=str(m), value=str(m.id)) for m in interaction.user.voice.channel.members if m.id != interaction.user.id]
    select = Select(options=options, placeholder="Who do i kick?", max_values=len(options))

    async def select_callback(inter: Interaction) -> None: 
     for values in select.values: 
      member = inter.guild.get_member(int(values))
      await member.move_to(channel=None, reason=f"Member kicked from voice channel by {inter.user}")
     await inter.response.edit_message(content=f"kicked {len(select.values)} members from your voice channel", view=None)
    select.callback = select_callback 
    view = View()
    view.add_item(select)
    await interaction.response.send_message("Use the dropdown menu to select the members", view=view, ephemeral=True) 
...
class VoiceMasterView(View): 
  ...
  async def add_button(self, guild: Guild, action: str, /, *, label: str="", emoji=None, style: str="gray"): 
    ...
    await self.bot.db.execute("INSERT INTO vm_buttons VALUES ($1,$2,$3,$4,$5)", guild.id, action, label, emoji, style)
