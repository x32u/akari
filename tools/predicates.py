...
def create_reskin():
 async def predicate(ctx: PretendContext):
  ...
  if not check: 
   await ctx.bot.db.execute("INSERT INTO reskin VALUES ($1,$2,$3)", ctx.author.id, ctx.bot.user.name, ctx.bot.user.display_avatar.url)
  ...
  if check: 
   if check['dice']:
    if datetime.datetime.now().timestamp() < check['dice']: 
     await ctx.economy_send(f"You can **dice** again **{ctx.bot.humanize_date(datetime.datetime.fromtimestamp(check['dice']))}**")
     return False 
  return True 
 return check(predicate)
